...
export const Dashboard: React.FC = () => {
  ...
  // Fetch all dashboard data when board, committee, date range, or period changes
  useEffect(() => {
    if (boardId) {
      setLoading(true);
      // Simulate API call with timeout
      setTimeout(() => {
        const data = getDashboardMetrics(boardId, activeCommittee, dateRange);
        const trends = getMeetingTrendsData(boardId, activeCommittee, dateRange, period);
        const distribution = getMeetingDistribution(boardId, activeCommittee, dateRange);
        const meetings = getUpcomingMeetings(boardId, activeCommittee, dateRange);
        const activity = getRecentActivity(boardId, activeCommittee, 8);
        const stats = getQuickStats(boardId, activeCommittee);
  ...
  }, [boardId, activeCommittee, dateRange, period]);
    ...
    <div>
      ...
      <Row justify="space-between" align="middle" style={{ marginBottom: 16 }} wrap>
        <Col xs={24} md={12}>
          ...
          <Text type="secondary">
            {activeCommittee === 'all' 
              ? `Viewing all boards and committees in ${currentBoard.shortName}`
              : activeCommittee === 'board'
              ? `Viewing ${currentBoard.shortName} board`
              : `Viewing ${currentBoard.shortName} - Committee`
            }
          </Text>
        </Col>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
        <Col xs={24} lg={12}>
          <Card
            ...
            extra={<a href={`/${boardId}/meetings`}>View All <RightOutlined /></a>}
            styles={{ body: { padding: '16px' } }}
            style={{ height: '100%' }}
          >
            <UpcomingMeetings
